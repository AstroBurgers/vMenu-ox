...
namespace vMenuClient.menus
{
    public class VehicleOptions : BaseScript
    {
        ...
        public bool VehicleInfiniteFuel { get; private set; }
        public bool VehicleShowHealth { get; private set; }
        public bool VehicleFrozen { get; private set; }
        public bool VehicleTorqueMultiplier { get; private set; }
        public bool VehiclePowerMultiplier { get; private set; }
        ...
        public bool VehicleCloseAllDoorsCooldown { get; private set; }
        ...
        private static void HandleVehicleSoundChange(string bagName, string key, dynamic value, int reserved,
            bool replicated)
        {
        ...
        private async Task HandleVehicleStat(MenuItem item, string field, string title, string suffix = "",
            double min = -1f, double max = -1f)
        {
            ...
            var current = isInteger
                ? GetVehicleHandlingInt(veh.Handle, "CHandlingData", field)
        ...
        private void CreateMenu()
        {
            ...
            var vehGodMenuBtn =
                new MenuItem("God Mode Options", "Enable or disable specific damage types.") { Label = "→→→" };
            MenuController.AddSubmenu(menu, vehGodMenu);

            // Create Checkboxes.
            var vehicleGod = new MenuCheckboxItem("Vehicle God Mode",
                ...
                VehicleGodMode);
            var vehicleNeverDirty = new MenuCheckboxItem("Keep Vehicle Clean",
                ...
                VehicleNeverDirty);
            var vehicleBikeSeatbelt = new MenuCheckboxItem("Bike Seatbelt",
                "Prevents you from being knocked off your bike, bicyle, ATV or similar.", VehicleBikeSeatbelt);
            var vehicleEngineAO = new MenuCheckboxItem("Engine Always On",
                "Keeps your vehicle engine on when you exit your vehicle.", VehicleEngineAlwaysOn);
            var vehicleNoTurbulence = new MenuCheckboxItem("Disable Plane Turbulence",
                "Disables the turbulence for all planes.", DisablePlaneTurbulence);
            var vehicleNoTurbulenceHeli = new MenuCheckboxItem("Disable Helicopter Turbulence",
                "Disables the turbulence for all helicopters.", DisableHelicopterTurbulence);
            var vehicleSetAnchor = new MenuCheckboxItem("Anchor Boat",
                "Only works if the current vehicle is a boat and its position is valid for anchoring", AnchorBoat);
            var vehicleNoSiren = new MenuCheckboxItem("Disable Siren",
                "Disables your vehicle's siren. Only works if your vehicle actually has a siren.", VehicleNoSiren);
            var vehicleNoBikeHelmet = new MenuCheckboxItem("No Bike Helmet",
                "No longer auto-equip a helmet when getting on a bike or quad.", VehicleNoBikeHelemet);
            var vehicleFreeze =
                new MenuCheckboxItem("Freeze Vehicle", "Freeze your vehicle's position.", VehicleFrozen);
            var torqueEnabled = new MenuCheckboxItem("Enable Torque Multiplier",
                "Enables the torque multiplier selected from the list below.", VehicleTorqueMultiplier);
            var powerEnabled = new MenuCheckboxItem("Enable Power Multiplier",
                "Enables the power multiplier selected from the list below.", VehiclePowerMultiplier);
            var brakeEnabled = new MenuCheckboxItem("Enable Brake Multiplier",
                "Enables the brake multiplier selected from the list below.", VehicleBrakeMultiplier);
            var highbeamsOnHonk = new MenuCheckboxItem("Flash Highbeams On Honk",
                ...
                FlashHighbeamsOnHonk);
            var showHealth = new MenuCheckboxItem("Show Vehicle Health", "Shows the vehicle health on the screen.",
                VehicleShowHealth);
            var infiniteFuel = new MenuCheckboxItem("Infinite Fuel",
                "Enables or disables infinite fuel for this vehicle, only works if FRFuel is installed.",
                VehicleInfiniteFuel);

            // Create buttons.
            var fixVehicle = new MenuItem("Repair Vehicle",
            ...
            var setLicensePlateText =
                new MenuItem("Set License Plate Text", "Enter a custom license plate for your vehicle.");
            var modMenuBtn = new MenuItem("Mod Menu", "Tune and customize your vehicle here.") { Label = "→→→" };
            var handlingMenuBtn = new MenuItem("Handling Menu", "Edit your vehicle's handle here.") { Label = "→→→" };
            var doorsMenuBtn =
                new MenuItem("Vehicle Doors", "Open, close, remove and restore vehicle doors here.") { Label = "→→→" };
            var windowsMenuBtn = new MenuItem("Vehicle Windows",
                "Roll your windows up/down or remove/restore your vehicle windows here.") { Label = "→→→" };
            var componentsMenuBtn =
                new MenuItem("Vehicle Extras", "Add/remove vehicle components/extras.") { Label = "→→→" };
            var liveriesMenuBtn =
                new MenuItem("Vehicle Liveries", "Style your vehicle with fancy liveries!") { Label = "→→→" };
            var colorsMenuBtn = new MenuItem("Vehicle Colors",
                "Style your vehicle even further by giving it some ~g~Snailsome ~s~colors!") { Label = "→→→" };
            var underglowMenuBtn =
                new MenuItem("Vehicle Neon Kits", "Make your vehicle shine with some fancy neon underglow!")
                {
                    Label = "→→→"
                };
            var engineSoundMenuBtn =
                new MenuItem("Vehicle Engine Sound", "Modify your vehicle's engine sound!") { Label = "→→→" };
            var vehicleInvisible = new MenuItem("Toggle Vehicle Visibility",
            ...
            var radioStations = new MenuListItem("Default radio station", stationNames, radioIndex,
                "Select a defalut radio station to be set when spawning new car");

            var tiresList = new List<string>()
            {
                "All Tires",
                "Tire #1",
                "Tire #2",
            ...
            var vehicleTiresList = new MenuListItem("Fix / Destroy Tires", tiresList, 0,
            ...
            var setDirtLevel = new MenuListItem("Set Dirt Level", dirtlevel, 0,
                "Select how much dirt should be visible on your vehicle, press ~r~enter~s~ " +
            ...
            var setLicensePlateType = new MenuListItem("License Plate Type", licensePlates, 0,
                "Choose a license plate type and press ~r~enter ~s~to apply " +
                "it to your vehicle.");
            ...
            var torqueMultiplier = new MenuListItem("Set Engine Torque Multiplier", torqueMultiplierList, 0,
            ...
            var powerMultiplier = new MenuListItem("Set Engine Power Multiplier", powerMultiplierList, 0,
                "Set the engine power multiplier.");
            var speedLimiterOptions = new List<string>() { "Set", "Reset", "Custom Speed Limit" };
            var speedLimiter = new MenuListItem("Speed Limiter", speedLimiterOptions, 0,
            ...
            if (IsAllowed(Permission.VOGod)) // GOD MODE
            {
                ...
                var godInvincible = new MenuCheckboxItem("Invincible",
                    "Makes the car invincible. Includes fire damage, explosion damage, collision damage and more.",
                    VehicleGodInvincible);
                var godEngine = new MenuCheckboxItem("Engine Damage", "Disables your engine from taking any damage.",
                    VehicleGodEngine);
                var godVisual = new MenuCheckboxItem("Visual Damage",
                    ...
                    VehicleGodVisual);
                var godStrongWheels = new MenuCheckboxItem("Strong Wheels",
                ...
                var godRamp = new MenuCheckboxItem("Ramp Damage",
                    "Disables vehicles such as the Ramp Buggy from taking damage when using the ramp.", VehicleGodRamp);
                var godAutoRepair = new MenuCheckboxItem("~r~Auto Repair",
            ...
            if (IsAllowed(Permission.VONoSiren) &&
                !GetSettingsBool(Setting.vmenu_use_els_compatibility_mode)) // DISABLE SIREN
            {
            ...
            menu.OnCheckboxChange += (sender, item, index, _checked) =>
            {
                ...
                else if (item == infiniteFuel)
                {
                    ...
                    var actionData = new Dictionary<string, object> { ["enabled"] = _checked };
            ...
            menu.OnListItemSelect += async (sender, item, listIndex, itemIndex) =>
            {
                ...
                else if (item == vehicleLights)
                {
                    if (Game.PlayerPed.IsInVehicle())
                    {
                        ...
                        var state = GetVehicleIndicatorLights(veh.Handle) %
                                    4; // 0 = none, 1 = left, 2 = right, 3 = both
                    ...
                    if (Game.PlayerPed.IsInVehicle())
                    {
                        ...
                        if (vehicle != null && vehicle.Exists())
                        {
                            if (listIndex == 0) // Set
                            {
                                ...
                                {
                                    Notify.Info(
                                        $"Vehicle speed is now limited to ~b~{Math.Round(vehicle.Speed * 3.6f, 1)} KPH~s~.");
                                }
                                else // mph
                                {
                                    Notify.Info(
                                        $"Vehicle speed is now limited to ~b~{Math.Round(vehicle.Speed * 2.237f, 1)} MPH~s~.");
                                }

                            }
                            else if (listIndex == 1) // Reset
                            {
                                SetEntityMaxSpeed(vehicle.Handle,
                                    500.01f); // Default max speed seemingly for all vehicles.
                            ...
                            else if (listIndex == 2) // custom speed
                            {
                                ...
                                if (!string.IsNullOrEmpty(inputSpeed))
                                {
                                    if (float.TryParse(inputSpeed, out var outFloat))
                                    {
                                        ...
                                        {
                                            Notify.Info(
                                                $"Vehicle speed is now limited to ~b~{Math.Round(outFloat * 3.6f, 1)} KPH~s~.");
                                        }
                                        else // mph
                                        {
                                            Notify.Info(
                                                $"Vehicle speed is now limited to ~b~{Math.Round(outFloat * 2.237f, 1)} MPH~s~.");
                                        }
                                    }
                                    else if (int.TryParse(inputSpeed, out var outInt))
                                    {
                                        ...
                                        {
                                            Notify.Info(
                                                $"Vehicle speed is now limited to ~b~{Math.Round(outInt * 3.6f, 1)} KPH~s~.");
                                        }
                                        else // mph
                                        {
                                            Notify.Info(
                                                $"Vehicle speed is now limited to ~b~{Math.Round(outInt * 2.237f, 1)} MPH~s~.");
                                        }
                                    }
                                    else
                                    {
            ...
            var vehicleEnveffScale = new MenuSliderItem("Vehicle Enveff Scale",
                "This works on certain vehicles only, like the besra for example. It 'fades' certain paint layers.", 0,
                20, 10, true);
            ...
            async void HandleItemSelect(Menu menu, MenuItem menuItem, int itemIndex)
            {
                ...
                if (veh != null && veh.Exists() && !veh.IsDead && veh.Driver == Game.PlayerPed)
                {
                    ...
                    var vehicleType = (menu == primaryColorsMenu) ? 1 : 2;
                    ...
                    var r = Convert.ToInt32(chosenColour.Substring(0, 2), 16);
                    var g = Convert.ToInt32(chosenColour.Substring(2, 2), 16);
                    var b = Convert.ToInt32(chosenColour.Substring(4, 2), 16);
            ...
            {
                var numVehColors = GetNumberOfVehicleColours(GetVehicle().Handle);
                ...
                for (var i = 0; i < numVehColors; i++)
                {
                    colorOptions.Add($"Preset Color #{i + 1}");
                }

                var currentColor = GetVehicleColourCombination(GetVehicle().Handle);
            ...
            var removeDoorList =
                new MenuListItem("Remove Door", doors, 0, "Remove a specific vehicle door completely.");
            var deleteDoors = new MenuCheckboxItem("Delete Removed Doors",
                    ...
                    if (veh != null && veh.Exists() && !veh.IsDead)
                    {
                        if (veh.Driver == Game.PlayerPed)
                        {
                            ...
                            if (liveryCount > 0)
                            {
                                ...
                                var liveryListItem = new MenuListItem("Set Livery", liveryList,
            ...
            var submergedBtn = new MenuItem("Percent Submerged",
                "Changes the percentage of the vehicle height that floats.");
            var driveBiasBtn = new MenuItem("Drive Bias Front", "Determines whether a vehicle is FWD, RWD or 4WD.");
            var driveGearsBtn = new MenuItem("Drive Gears",
                "Changes the number of forward speeds a transmission contains.");
            ...
            var clutchRateUpBtn =
                new MenuItem("Clutch Change Rate Up Shift", "Changes how fast a vehicle's up shifts are.");
            var clutchRateDownBtn = new MenuItem("Clutch Change Rate Down Shift",
                "Changes how fast a vehicle's down shifts are.");
            var driveMaxFlatBtn = new MenuItem("Drive Max Flat Velocity", "Changes the speed at redline in top gear.");
            var brakeForceBtn = new MenuItem("Brake Force",
                "Changes the multiplier for the game's calculation of deceleration.");
            var brakeBiasBtn = new MenuItem("Brake Bias Front",
                "Changes the distribution of the braking force between front and rear.");
            var handbrakeBtn = new MenuItem("Handbrake Force", "Changes the braking power of the handbrake.");
            var steeringLockBtn = new MenuItem("Steering Lock",
                "Changes the maximum angle that steered wheels wil be able to turn.");
            var tractionCurveMaxBtn = new MenuItem("Traction Curve Max",
                "Changes maximum cornering / acceleration coefficient of grip.");
            var tractionCurveMinBtn = new MenuItem("Traction Curve Min",
                "Changes sliding cornering / acceleration coefficient of grip.");
            var tractionCurveLateralBtn = new MenuItem("Traction Curve Lateral",
                "Changes shape of lateral traction curve where lower values make grip more responsive and higher levels make grip less responsive.");
            var tractionSpringDeltaBtn = new MenuItem("Traction Spring Delta",
                "Changes the max distance of lateral sidewall travel in meters.");
            var lowSpeedTractionLossBtn = new MenuItem("Low Speed Traction Loss",
                "Changes how much traction is reduced at low speed, mostly affecting burnout.");
            var camberStiffBtn = new MenuItem("Camber Stiffness",
                "Changes how much vehicle is pushed towards its roll direction.");
            var tractionBiasFrontBtn = new MenuItem("Traction Bias Front",
                "Determines the distribution of traction from front to rear.");
            var tractionLossBtn = new MenuItem("Traction Loss",
                "Changes how much traction is affected by material grip differences.");
            var suspensionForceBtn = new MenuItem("Suspension Force", "Affects how strong suspension is.");
            var suspensionCompBtn =
                new MenuItem("Suspension Compression", "Affects the damping during strut comrpession.");
            var suspensionReboundBtn = new MenuItem("Suspension Rebound", "Affects the damping during strut rebound.");
            var suspensionUpperBtn = new MenuItem("Suspension Upper Limit",
                "Changes how far the wheels can move up from their original position.");
            var suspensionLowerBtn = new MenuItem("Suspension Lower Limit",
                "Changes how far the wheels can move down from their original position.");
            var suspensionRaiseBtn = new MenuItem("Suspension Raise",
                "Changes the amount the suspension raises the body off the wheels.");
            var suspensionBiasBtn = new MenuItem("Suspension Bias Front",
                "Changes the force damping scale from front to back.");
            var antiRollBtn = new MenuItem("Anti Rollbar Force",
                "Changes the spring constant where larger numbers reduce body roll.");
            var antiRollBiasBtn = new MenuItem("Anti Rollbar Bias Front",
                "Changes the bias between the front and rear of the antiroll bar.");
            var rollCentreFrontBtn = new MenuItem("Roll Centre Height Front",
                "Changes the roll centre height for the front axle from the road in metres.");
            var rollCentreRearBtn = new MenuItem("Roll Centre Height Rear",
                "Change the roll center height for the rear axle from the road in metres.");
            var collisionDmgBtn =
                new MenuItem("Collision Damage", "Changes the multiplier for the game's collision damage.");
            var weaponDmgBtn = new MenuItem("Weapon Damage", "Changes the multiplier for the game's weapon damage.");
            var deformDmgBtn = new MenuItem("Deformation Damage",
                "Changes the multiplier for the game's visual deformation.");
            var engineDmgBtn = new MenuItem("Engine Damage", "Changes the multiplier for the game's engine damage.");
            var petrolVolBtn = new MenuItem("Petrol Tank Volume",
                "Changes the amount of petrol in the vehicle's petrol tank.");
            ...
            VehicleHandlingMenu.OnMenuOpen += (sender) =>
            {
                ...
                if (veh != null && veh.Exists())
                {
                    ...
                    double currentDownforce =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fDownForceModifier");
                    ...
                    double currentDriveForce =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fInitialDriveForce");
                    ...
                    double currentClutchRateUp =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fClutchChangeRateScaleUpShift");
                    clutchRateUpBtn.Label = currentClutchRateUp.ToString("F2") + "x";

                    double currentClutchRateDown =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fClutchChangeRateScaleDownShift");
                    clutchRateDownBtn.Label = currentClutchRateDown.ToString("F2") + "x";

                    double currentDriveMaxFlat =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fInitialDriveMaxFlatVel");
                    ...
                    double currentHandbrakeForce =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fHandBrakeForce");
                    ...
                    double currentTractionCurveMax =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fTractionCurveMax");
                    tractionCurveMaxBtn.Label = currentTractionCurveMax.ToString("F2");

                    double currentTractionCurveMin =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fTractionCurveMin");
                    tractionCurveMinBtn.Label = currentTractionCurveMin.ToString("F2");

                    double currentTractionCurveLateral =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fTractionCurveLateral");
                    tractionCurveLateralBtn.Label = currentTractionCurveLateral.ToString("F2");

                    double currentTractionSpringDelta =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fTractionSpringDeltaMax");
                    tractionSpringDeltaBtn.Label = currentTractionSpringDelta.ToString("F2");

                    double currentLowSpeedTractionLoss =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fLowSpeedTractionLossMult");
                    lowSpeedTractionLossBtn.Label = currentLowSpeedTractionLoss.ToString("F2");

                    double currentCamberStiff =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fCamberStiffnesss");
                    camberStiffBtn.Label = currentCamberStiff.ToString("F2");

                    double currentTractionBiasFront =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fTractionBiasFront");
                    tractionBiasFrontBtn.Label = currentTractionBiasFront.ToString("F2");

                    double currentTractionLoss =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fTractionLossMult");
                    tractionLossBtn.Label = currentTractionLoss.ToString("F2");

                    double currentSuspensionForce =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fSuspensionForce");
                    suspensionForceBtn.Label = currentSuspensionForce.ToString("F2");

                    double currentSuspensionCompression =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fSuspensionCompDamp");
                    suspensionCompBtn.Label = currentSuspensionCompression.ToString("F2");

                    double currentCompressionRebound =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fSuspensionReboundDamp");
                    suspensionReboundBtn.Label = currentCompressionRebound.ToString("F2");

                    double currentSuspensionUpper =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fSuspensionUpperLimit");
                    suspensionUpperBtn.Label = currentSuspensionUpper.ToString("F2");

                    double currentSuspensionLower =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fSuspensionLowerLimit");
                    suspensionLowerBtn.Label = currentSuspensionLower.ToString("F2");

                    double currentSuspensionRaise =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fSuspensionRaise");
                    suspensionRaiseBtn.Label = currentSuspensionRaise.ToString("F2");

                    double currentSuspensionBias =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fSuspensionBiasFront");
                    ...
                    double currentAntiRollBias =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fAntiRollBarBiasFront");
                    antiRollBiasBtn.Label = currentAntiRollBias.ToString("F2");

                    double currentRollCentreFront =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fRollCentreHeightFront");
                    rollCentreFrontBtn.Label = currentRollCentreFront.ToString("F2");

                    double currentRollCentreRear =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fRollCentreHeightRear");
                    rollCentreRearBtn.Label = currentRollCentreRear.ToString("F2");

                    double currentCollisionDmg =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fCollisionDamageMult");
                    ...
                    double currentDeformDmg =
                        GetVehicleHandlingFloat(veh.Handle, "CHandlingData", "fDeformationDamageMult");
            ...
            var soundMenuList = new MenuListItem("Set Engine Sound", soundNameList, 0,
                "Select the vehicle engine sound here");
            ...
            VehicleEngineSoundMenu.OnItemSelect += (sender, item, index) =>
            {
                ...
                else
                {
                    ...
                    var vehicle = VehToNet(veh.Handle);
            ...
            VehicleEngineSoundMenu.OnListItemSelect += (sender, item, index, itemIndex) =>
            {
                ...
                else
                {
                    ...
                    var name = soundNameList[index];
                    var selected = soundList[index];
            ...
            {
                Vehicle vehicle;
                var checkDamageBeforeChangingExtras = GetSettingsBool(Setting.vmenu_prevent_extras_when_damaged) &&
                                                      !IsAllowed(Permission.VOBypassExtraDamage);
                ...
                var isTooDamaged = IsVehicleTooDamagedToChangeExtras(vehicle);

                menu.ClearMenuItems();

                if (isTooDamaged && !menuItems.Exists(i => i.Text.Contains("too damaged")))
                {
                    var spacer = GetSpacerMenuItem("Vehicle too damaged!",
            ...
            VehicleComponentsMenu.OnCheckboxChange += (sender, item, index, _checked) =>
            {
                ...
                if (vehicleExtras.TryGetValue(item, out var extra))
                {
                    ...
                    {
                        var isTooDamaged = IsVehicleTooDamagedToChangeExtras(veh);
            ...
            var underglowColor = new MenuListItem(GetLabelText("CMOD_NEON_1"), underglowColorsList, 0,
            ...
            menu.OnItemSelect += (sender, item, index) =>
            {
                ...
                if (item == underglowMenuBtn)
                {
                    ...
                    if (veh != null)
                    {
                        ...
                        {
                            underglowFront.Checked = veh.Mods.HasNeonLight(VehicleNeonLight.Front) &&
                                                     veh.Mods.IsNeonLightsOn(VehicleNeonLight.Front);
                            underglowBack.Checked = veh.Mods.HasNeonLight(VehicleNeonLight.Back) &&
                                                    veh.Mods.IsNeonLightsOn(VehicleNeonLight.Back);
                            underglowLeft.Checked = veh.Mods.HasNeonLight(VehicleNeonLight.Left) &&
                                                    veh.Mods.IsNeonLightsOn(VehicleNeonLight.Left);
                            underglowRight.Checked = veh.Mods.HasNeonLight(VehicleNeonLight.Right) &&
                                                     veh.Mods.IsNeonLightsOn(VehicleNeonLight.Right);
            ...
            VehicleUnderglowMenu.OnCheckboxChange += (sender, item, index, _checked) =>
            {
                if (Game.PlayerPed.IsInVehicle())
                {
                    ...
                    if (veh.Mods.HasNeonLights)
                    {
                        ...
                        {
                            veh.Mods.SetNeonLightsOn(VehicleNeonLight.Left,
                                veh.Mods.HasNeonLight(VehicleNeonLight.Left) && _checked);
                        }
                        else if (item == underglowRight)
                        {
                            veh.Mods.SetNeonLightsOn(VehicleNeonLight.Right,
                                veh.Mods.HasNeonLight(VehicleNeonLight.Right) && _checked);
                        }
                        else if (item == underglowBack)
                        {
                            veh.Mods.SetNeonLightsOn(VehicleNeonLight.Back,
                                veh.Mods.HasNeonLight(VehicleNeonLight.Back) && _checked);
                        }
                        else if (item == underglowFront)
                        {
                            veh.Mods.SetNeonLightsOn(VehicleNeonLight.Front,
                                veh.Mods.HasNeonLight(VehicleNeonLight.Front) && _checked);
                        }
                    }
                }
            };
            ...
            if (veh != null && veh.Exists() && !veh.IsDead)
            {
                ...
                foreach (var mod in mods)
                {
                    ...
                    for (var x = 0; x < mod.ModCount; x++)
                    {
                        ...
                        name = mod.GetLocalizedModName(x) != ""
                            ? $"{ToProperString(mod.GetLocalizedModName(x))} {currentItem}"
                            : $"{typeName} #{x} {currentItem}";
                        modlist.Add(name);
                    }
                ...
                var vehicleWheelType = new MenuListItem("Wheel Type", wheelTypes,
                    MathUtil.Clamp(GetVehicleWheelType(veh.Handle), 0, 12),
                    $"Choose a ~y~wheel type~s~ for your vehicle.");
                if (!veh.Model.IsBoat && !veh.Model.IsHelicopter && !veh.Model.IsPlane && !veh.Model.IsBicycle &&
                    !veh.Model.IsTrain)
                {
                ...
                var toggleCustomWheels = new MenuCheckboxItem("Toggle Custom Wheels",
                    "Press this to add or remove ~y~custom~s~ wheels.", GetVehicleModVariation(veh.Handle, 23));
                var xenonHeadlights = new MenuCheckboxItem("Xenon Headlights",
                    "Enable or disable ~b~xenon ~s~headlights.", IsToggleModOn(veh.Handle, 22));
                var turbo = new MenuCheckboxItem("Turbo", "Enable or disable the ~y~turbo~s~ for this vehicle.",
                    IsToggleModOn(veh.Handle, 18));
                var bulletProofTires = new MenuCheckboxItem("Bullet Proof Tires",
                    "Enable or disable ~y~bullet proof tires~s~ for this vehicle.",
                    !GetVehicleTyresCanBurst(veh.Handle));
                ...
                var headlightColor = new MenuListItem("Headlight Color",
                    [
                        "White",
                        "Blue",
                        "Electric Blue",
                ...
                var isLowGripAvailable = GetGameBuildNumber() >= 2372;
                var lowGripTires = new MenuCheckboxItem("Low Grip Tires",
                    "Enable or disable ~y~low grip tires~s~ for this vehicle.",
                    isLowGripAvailable ? GetDriftTyresEnabled(veh.Handle) : false);
                ...
                var tireSmokes = new List<string>()
                {
                    "Red",
                    "Orange",
                    "Yellow",
                ...
                var item = tireSmokeColors.ToList().Find((f) =>
                {
                    return f.Value[0] == smoker && f.Value[1] == smokeg && f.Value[2] == smokeb;
                });
                ...
                var tireSmoke = new MenuListItem("Tire Smoke Color", tireSmokes, index,
                ...
                var tireSmokeEnabled = new MenuCheckboxItem("Tire Smoke",
                    "Enable or disable ~y~tire smoke~s~ for your vehicle. ~h~~r~Important:~s~ When disabling tire smoke, you'll need to drive around before it takes affect.",
                    IsToggleModOn(veh.Handle, 20));
                VehicleModMenu.AddMenuItem(tireSmokeEnabled);

                // Create list for window tint
                var windowTints = new List<string>()
                {
                    "Stock [1/7]",
                    "None [2/7]",
                    "Limo [3/7]",
                ...
                var windowTint =
                    new MenuListItem("Window Tint", windowTints, currentTint, "Apply tint to your windows.");
                ...
                VehicleModMenu.OnCheckboxChange += (sender2, item2, index2, _checked) =>
                {
                    ...
                    {
                        SetVehicleMod(veh.Handle, 23, GetVehicleMod(veh.Handle, 23),
                            !GetVehicleModVariation(veh.Handle, 23));

                        // If the player is on a motorcycle, also change the back wheels.
                        if (IsThisModelABike((uint)GetEntityModel(veh.Handle)))
                        {
                            SetVehicleMod(veh.Handle, 24, GetVehicleMod(veh.Handle, 24),
                                GetVehicleModVariation(veh.Handle, 23));
                        }
                    }
                    // Toggle Tire Smoke
                ...
                VehicleModMenu.OnListIndexChange += (sender2, item2, oldIndex, newIndex, itemIndex) =>
                {
                    ...
                    else if (item2 == vehicleWheelType)
                    {
                        ...
                        var isBikeOrOpenWheel = (newIndex == 6 && veh.Model.IsBike) ||
                                                (newIndex == 10 && vehicleClass == 22);
                        var isNotBikeNorOpenWheel =
                            newIndex != 6 && !veh.Model.IsBike && newIndex != 10 && vehicleClass != 22;
        ...
        private readonly List<int[]> _VehicleNeonLightColors =
        [
        ...
        private System.Drawing.Color GetColorFromIndex(int index)
        {
            ...
            {
                return System.Drawing.Color.FromArgb(_VehicleNeonLightColors[index][0],
                    _VehicleNeonLightColors[index][1], _VehicleNeonLightColors[index][2]);
            }
        ...
        private int GetIndexFromColor()
        {
            ...
            return _VehicleNeonLightColors.Any(a => a[0] == r && a[1] == g && a[2] == b) ? _VehicleNeonLightColors.FindIndex(a => a[0] == r && a[1] == g && a[2] == b) : 0;
        }
        ...
        {
            var bodyHealth = vehicle.BodyHealth;
            var engineHealth = vehicle.EngineHealth;
